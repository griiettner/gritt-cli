...
Read [tkt](../tkt/SKILL.md) first. Ticket resolution: [tkt/store](../tkt/store/SKILL.md). Autonomy and goal tracking: [tkt/autonomy](../tkt/autonomy/SKILL.md). Closing rules: [tkt/completion](../tkt/completion/SKILL.md). Code: [dev](../dev/SKILL.md) and one of its sub-skills. Load [`write`](../write/SKILL.md) on `report.md` and update files.
...
6. Review your own diff against the acceptance criteria and the scope before writing the report: unrelated files, missing tests, secrets, and any plan rule you could not satisfy. When the harness offers a separate review agent, use it here and treat its verdict like a reviewer's.
